...
from ecomapp.models import (
    LoginLog, Employee, MenuList,City, Company, UserPermission,ProductMainCategory
   
)

from .models import ProductMainCategory 
from .models import ProductSubCategory 
from .models import ProductChildCategory 
from .models import AttributeList 
from .models import AttributeValueList 

from .forms import ProductMainCategoryForm 
from .forms import ProductSubCategoryForm 
from .forms import ProductChildCategoryForm 
from .forms import AttributeListForm 
from .forms import AttributeValueListForm 
...
def user_login(request):
    ...
    if request.method == 'POST':
        ...
        if len(username) <= 4 or len(password) <= 5:
            error_message = "Username or password length too much short!"
        ...
        if user is not None:
            ...
            if employee:
                request.session['employee_id_number'] = employee.employee_id_number
                request.session['employee_id'] = employee.id
            else:
                request.session['employee_id_number'] = ""
